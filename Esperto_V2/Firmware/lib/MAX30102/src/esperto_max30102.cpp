...
boolean MAX30102::begin(TwoWire &wirePort, uint32_t i2cSpeed, uint8_t i2caddr) {
  ...
  // Initial Communciation and Verification
...
/* Set which LEDs are used for sampling -- Red only, RED+IR only, or custom. See pg19 */
...
/* Resets all points to start in a known state - Page 15 recommends clearing FIFO before beginning a read */
...
/* Die Temperature - Returns temp in C */
...
{
  // Only advance the tail if new data is available
  if(available()) 
  {
    sense.tail++;
...
/*
  Polls the sensor for new data - Call regularly
